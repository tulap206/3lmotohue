...
export function DashboardSidebar({ children }: SidebarProps) {
  ...
  const nav = (
    <>
      <div className="flex items-center gap-3 h-[4.5rem] px-5 border-b border-slate-100 shrink-0">
        ...
        <div className="min-w-0">
          <p className="text-body font-semibold text-slate-900 truncate leading-tight">3L Moto</p>
          <p className="text-meta truncate">Cho thuê xe</p>
        </div>
        <button
          type="button"
          onClick={() => setMobileOpen(false)}
          className="lg:hidden ml-auto p-2 rounded-[var(--radius-control)] hover:bg-slate-100 ui-transition touch-target"
      ...
      <div className="mt-auto border-t border-slate-100 p-3 space-y-1 shrink-0">
        {user?.role === "admin" && (
          <Link
        ...
        {user && (
          <button
            ...
            className="w-full flex items-center gap-3 h-12 px-3 rounded-[var(--radius-control)] hover:bg-slate-50 ui-transition text-left"
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block text-body font-semibold text-slate-800 truncate">{user.displayName}</span>
              <span className="block text-meta truncate">{user.role === "admin" ? "Admin" : "Nhân viên"}</span>
            </span>
          </button>
        )}
        ...
        <button
          ...
          className="w-full flex items-center gap-3 h-11 px-3 rounded-[var(--radius-control)] text-body font-medium text-slate-600 hover:bg-rose-50 hover:text-rose-600 ui-transition"
        >
  ...
  return (
    <div className="flex min-h-screen dashboard-bg">
      ...
      <aside
        className={cn(
          "fixed left-0 top-0 z-50 h-screen w-[16.5rem] bg-white border-r border-slate-200/90 flex flex-col ui-transition",
